 ...
 * one-directional pico/util/queue.h queue_t instances (each internally spinlock-protected, safe
 * to use from either core without any additional locking on our part):
...
void buffer_callback(hw_interface::audio_buffer_t *buffer_0, hw_interface::audio_buffer_t *buffer_1)
{
    ...
    // effect_controller.Process(frame,frame,frame.n_frames);
    ...
    while (true)
    {
        ...
        while (input_event_queue_.TryPop(command))
        {
            if (command.type == hw_interface::InputEventType::kNavigationEvent)
            {
                ...
                audio_codec.Stop();
                audio_player.Stop();
                constexpr size_t kMaxLoadFileTry = 3;

                bool load_success = false;

                for (size_t i = 0; i < kMaxLoadFileTry; i++)
                {
                    ...
                    if (ret == 0 && selected_path[0] != '\0' && audio_player.LoadFile(selected_path) == 0)
                    {
                        telemetry_queue_.PushFileManagerState(file_manager.GetCurrentBankIndex(), file_manager.GetNumberOfBanks(),
                                                              ...
                                                              file_manager.GetNumberOfFileInCurrentBank(), file_manager.GetFileName(), false);
...
int InitFileSystem()
{
    ...
    // Own the app-wide singleton IFileSystem instance (see app::GetFileSystem() in IFileSystem.h)
